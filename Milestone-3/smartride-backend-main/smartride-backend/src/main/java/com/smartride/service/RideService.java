...
public class RideService {
    ...
    public Ride postRide(Long driverId, RideRequest request) {
        ...
        // ✅ FIX: Driver sets price per seat directly
        ride.setPricePerSeat(request.getPricePerSeat());
        log.info("Ride Price Per Seat: {}", ride.getPricePerSeat());

        if (request.getRouteStops() != null && !request.getRouteStops().isEmpty()) {
            ride.setRouteStops(String.join(",", request.getRouteStops()));
        }
    ...
    public List<RideResponse> searchRides(String source, String destination,
                                          LocalDate date, Integer seats, Double maxPrice, String vehicleType, Double minRating) {
        List<Ride> rides;

        if (date != null) {
            rides = rideRepository.findByRideDateAndStatus(date, Ride.RideStatus.ACTIVE);
        } else {
            rides = rideRepository.findByStatus(Ride.RideStatus.ACTIVE);
        }

        int requestedSeats = seats != null && seats > 0 ? seats : 1;
        
        return rides.stream()
                .filter(r -> r.getAvailableSeats() >= requestedSeats)
                .filter(r -> matchRoute(r, source, destination))
                .map(ride -> convertToResponse(ride, requestedSeats))
                .filter(r -> maxPrice == null || r.getPricePerSeat() <= maxPrice)
                .filter(r -> vehicleType == null || vehicleType.isEmpty() || (r.getDriverVehicleType() != null && r.getDriverVehicleType().equalsIgnoreCase(vehicleType)))
                .filter(r -> minRating == null || (r.getDriverRating() != null && r.getDriverRating() >= minRating))
                .collect(Collectors.toList());
    }

    private boolean matchRoute(Ride ride, String searchSource, String searchDest) {
        String sSrc = searchSource.trim().toLowerCase();
        String sDest = searchDest.trim().toLowerCase();
        
        java.util.List<String> route = new java.util.ArrayList<>();
        route.add(ride.getSource().trim().toLowerCase());
        if (ride.getRouteStops() != null && !ride.getRouteStops().isEmpty()) {
            for (String stop : ride.getRouteStops().split(",")) {
                route.add(stop.trim().toLowerCase());
            }
        }
        route.add(ride.getDestination().trim().toLowerCase());

        int srcIndex = -1;
        int destIndex = -1;
        
        for (int i = 0; i < route.size(); i++) {
            if (srcIndex == -1 && route.get(i).contains(sSrc)) {
                srcIndex = i;
            }
            if (srcIndex != -1 && i > srcIndex && route.get(i).contains(sDest)) {
                destIndex = i;
                break;
            }
        }

        return srcIndex != -1 && destIndex != -1;
    }
    ...
    public Ride cancelRide(Long rideId, Long driverId) {
        ...
        // Notify passengers
        bookingRepository.findByRideIdAndStatus(rideId, Booking.BookingStatus.CONFIRMED).forEach(b -> {
            b.setStatus(Booking.BookingStatus.CANCELLED);
            bookingRepository.save(b);
            notificationService.saveAndEmit(
                b.getPassengerId(), driverId, "CANCELLED", rideId,
                "Your ride from " + ride.getSource() + " to " + ride.getDestination() + " has been cancelled by the driver."
            );
        });

        bookingRepository.findByRideIdAndStatus(rideId, Booking.BookingStatus.PENDING).forEach(b -> {
            b.setStatus(Booking.BookingStatus.CANCELLED);
            bookingRepository.save(b);
            notificationService.saveAndEmit(
                b.getPassengerId(), driverId, "CANCELLED", rideId,
                "Your pending ride from " + ride.getSource() + " to " + ride.getDestination() + " has been cancelled."
            );
        });
    ...
    public Ride startRide(Long rideId, Long driverId) {
        ...
        if (!ride.getDriverId().equals(driverId)) {
            throw new RuntimeException("You can only start your own rides");
        }

        if (ride.getStatus() == Ride.RideStatus.CANCELLED) {
            throw new RuntimeException("Cannot start a cancelled ride");
        }
        ...
        if (ride.getStatus() == Ride.RideStatus.IN_PROGRESS) {
            throw new RuntimeException("This ride is already in progress");
        }

        ride.setStatus(Ride.RideStatus.IN_PROGRESS);
        Ride ongoingRide = rideRepository.save(ride);

        bookingRepository.findByRideIdAndStatus(rideId, Booking.BookingStatus.CONFIRMED).forEach(b -> {
            notificationService.saveAndEmit(
                b.getPassengerId(), driverId, "STARTED", rideId,
                "Your ride from " + ride.getSource() + " to " + ride.getDestination() + " has just started!"
            );
        });
     ...
     * ============================================================
     * Marks the ride as ACCEPTED, allowing passengers to proceed with payment.
     * ============================================================
     */
    @Transactional
    public Ride acceptRide(Long rideId, Long driverId) {
        ...
        if (!ride.getDriverId().equals(driverId)) {
            throw new RuntimeException("You can only accept your own rides");
        }

        if (ride.getStatus() == Ride.RideStatus.CANCELLED) {
            throw new RuntimeException("Cannot accept a cancelled ride");
        }
    ...
    private RideResponse convertToResponse(Ride ride, int seatsRequested) {
        ...
        var fareEstimate = fareService.calculateFare(distanceKm, baseFare, farePerKm, seats, totalSeats, ride.getPricePerSeat());
        ...
        if (ride.getRouteStops() != null && !ride.getRouteStops().isEmpty()) {
            response.setRouteStops(java.util.Arrays.asList(ride.getRouteStops().split(",")));
        } else {
