...
public class BookingService {
    ...
    public Booking bookRide(Long userId, BookingRequest request) {
        ...
        // SINGLE SOURCE OF TRUTH: Calculate passenger's actual distance if coordinates are given
        ...
        if (request.getOriginLat() != null && request.getOriginLng() != null
            ...
            distanceKm = distanceService.getDistanceInKm(
        ...
        double finalPassengerFare = fareService
                .calculateFare(distanceKm, baseFare, farePerKm, request.getSeatsToBook(), totalSeats, ride.getPricePerSeat())
                .getPassengerFare();

        // VALIDATION CHECK
        if (finalPassengerFare <= 0) {
            throw new RuntimeException("Final fare calculation failed. Amount must be greater than 0.");
        }

        // LOG
        log.info("FINAL PAYMENT AMOUNT: ₹" + finalPassengerFare);
        ...
        booking.setTotalFare(finalPassengerFare);
        ...
        // Step 9: Save a ride_booking snapshot with distance and fare details
        ...
        rideBooking.setFare(finalPassengerFare);
    ...
    public BookingResponse convertBookingToResponse(Booking booking) {
        ...
        response.setRideStatus(ride.getStatus() != null ? ride.getStatus().name() : null);
