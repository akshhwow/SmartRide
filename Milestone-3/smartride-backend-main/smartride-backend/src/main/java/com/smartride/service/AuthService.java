...
public class AuthService {
    ...
    @Value("${app.dev.allow-otp-retrieval:false}")
    private boolean allowOtpRetrieval;
    ...
    public ApiResponse<String> register(RegisterRequest request) {
        ...
        try {
            ...
            // If email sending fails, auto-verify the user to allow login locally.
            // Still store the OTP so the user can verify via the verify-otp endpoint if needed.
            ...
            return ApiResponse.success(
                    "Account created and auto-verified (email could not be sent). " +
                            "Use OTP to verify if needed.",
                    otpCode
            );
        }
    }
    ...
    /**
     * Development helper: return the latest OTP for an email.
     * This is disabled by default (set app.dev.allow-otp-retrieval=true to enable).
     */
    public ApiResponse<String> getLatestOtpForEmail(String emailParam) {
        String email = emailParam != null ? emailParam.trim().toLowerCase() : "";
        
        if (!allowOtpRetrieval) {
            return ApiResponse.error("OTP retrieval is disabled. Enable app.dev.allow-otp-retrieval=true to use this.");
        }

        Optional<EmailOtp> otpOpt = emailOtpRepository.findByEmail(email).stream()
                .max(Comparator.comparing(EmailOtp::getCreatedAt));

        return otpOpt
                .map(otp -> ApiResponse.success("Latest OTP retrieved", otp.getOtpCode()))
                .orElseGet(() -> ApiResponse.error("No OTP found for this email."));
    }
    ...
    public ApiResponse<AuthResponse> login(LoginRequest request) {
        ...
        // Step 2: Check if email is verified (can be optionally disabled in config)
        if (requireEmailVerification && !user.getVerified()) {
            throw new RuntimeException("Please verify your email before logging in. Check your inbox for the verification code.");
        }
     ...
     */
    @Transactional
    public ApiResponse<String> resendOtp(String emailParam) {
    ...
    private AuthResponse.UserResponse convertToUserResponse(User user) {
        ...
        userResponse.setEmailVerified(user.getVerified());
