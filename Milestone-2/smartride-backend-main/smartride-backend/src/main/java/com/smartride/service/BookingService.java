...
import com.smartride.repository.RideBookingRepository;
...
public class BookingService {
    ...
    private final NotificationService notificationService; // NEW: Real-time notifications
    ...
    public Booking bookRide(Long userId, BookingRequest request) {
        ...
        // Step 5: Calculate total fare (consistent across UI, booking, payment)
        // Use the same FareService formula to avoid mismatches.
        ...
        double totalFare = fareService
                .calculateFare(distanceKm, baseFare, farePerKm, request.getSeatsToBook(), totalSeats)
                .getPassengerFare();
        ...
        // Booking is created before payment is completed.
        // Once payment is verified, we will mark it CONFIRMED.
        booking.setStatus(Booking.BookingStatus.PENDING);
        booking.setPaymentStatus(com.smartride.entity.PaymentStatus.PENDING);
        ...
        // Step 9: Optionally save a ride_booking snapshot with distance and fare details
        if (request.getOriginLat() != null && request.getOriginLng() != null
                && request.getDestinationLat() != null && request.getDestinationLng() != null) {

            double bookingDistanceKm = distanceService.getDistanceInKm(
            ...
            // Calculate fare breakdown using driver configuration
            double bookingBaseFare = ride.getBaseFare() != null ? ride.getBaseFare() : 0.0;
            double bookingFarePerKm = ride.getFarePerKm() != null ? ride.getFarePerKm() : 0.0;

            double passengerFare = fareService
                    .calculateFare(bookingDistanceKm, bookingBaseFare, bookingFarePerKm, request.getSeatsToBook(), ride.getTotalSeats())
                    .getPassengerFare();

            // Store the fare and distance snapshot for this booking
            com.smartride.entity.RideBooking rideBooking = new com.smartride.entity.RideBooking();
            rideBooking.setBooking(savedBooking);
            rideBooking.setDistanceKm(distanceKm);
            rideBooking.setFare(passengerFare);
            rideBooking.setPaymentId(savedBooking.getPaymentId());
            rideBooking.setPaymentStatus(savedBooking.getPaymentStatus());

            rideBookingRepository.save(rideBooking);
        }

        // NEW: Real-time notification emitted to driver
        ...
        // NEW: Real-time notification emitted to passenger
        notificationService.saveAndEmit(
                userId,
                userId,
                "BOOKING_CONFIRMED",
