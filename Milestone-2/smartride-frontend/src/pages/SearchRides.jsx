...
const SearchRides = () => {
  ...
  const geocodeAddress = async (address) => {
    // Use OpenStreetMap Nominatim to convert text address into coordinates
            ...
            {rides.map((ride) => (
              <div key={ride.id} className="ride-card">
                ...
                <div className="fare-section">
                  ...
                  >
                    {estimateLoading && estimatingRideId === ride.id ? '⏳ Estimating...' : '🧮 Estimate Fare'}
                  </button>
                  ...
                  {fareEstimates[ride.id] && (
                    <div className="fare-box">
                      <div className="fare-row">
                        <span>Distance</span>
                        <span>{fareEstimates[ride.id].distanceKm.toFixed(2)} km</span>
                      </div>
                      <div className="fare-row">
                        <span>Base Fare</span>
                        <span>₹{fareEstimates[ride.id].baseFare.toFixed(2)}</span>
                      </div>
                      <div className="fare-row">
                        <span>Fare / km</span>
                        <span>₹{fareEstimates[ride.id].farePerKm.toFixed(2)}</span>
                      </div>
                      <div className="fare-row">
                        <span>Total Fare</span>
                        <span>₹{fareEstimates[ride.id].totalFare.toFixed(2)}</span>
                      </div>
                      <div className="fare-row highlight">
                        <span>Your Fare</span>
                        <span>₹{fareEstimates[ride.id].passengerFare.toFixed(2)}</span>
                      </div>
                    </div>
                  )}
                </div>
